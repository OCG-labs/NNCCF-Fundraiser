...
if(!function_exists('wdf_fundraiser_panel')) {
	function wdf_fundraiser_panel($echo = true, $post_id = '', $context = '', $args = array() ) {
		...
		$content .= '<div class="'.$style.' wdf_fundraiser_panel">';
...
if(!function_exists('wdf_events')) {
	function wdf_events($echo = false, $post_id = '') {
		global $wdf, $post;
		$post_id = (empty($post_id) ? $post->ID : $post_id );
		if(!get_post($post_id))
			return;
		...
		$meta = get_post_custom($post_id);
		if(wdf_has_events($post_id)) {
			if(isset($meta['wdf_eventlevels'][0])) {
				$levels = '<div class="wdf_eventlevels">';
				$levels .= '<input type="hidden" name="wdf_eventlevel_name" value="" />';
				$levels .= '<input type="hidden" name="wdf_eventlevel_amounttype" value="" />';
				$levels .= '<input type="hidden" name="wdf_eventlevel_numattendees" value="" />';
				foreach($meta['wdf_eventlevels'] as $level) {
					$level = maybe_unserialize($level);

					foreach($level as $index => $data) {
						if (!isset($data['disable'])){
						$levels .= '<div class="wdf_event_item wdf_event_'.$index.'">
						              <input type="hidden" name="wdf_eventlevel_name_'.$index.'" value="'.$data['title'].'" />
						              <input type="hidden" name="wdf_eventlevel_amounttype_'.$index.'" value="'.$data['amounttype'].'" />
						              <input type="hidden" class="wdf_eventlevel_numattendees_'.$index.'" name="wdf_eventlevel_numattendees_'. $index .'" value="'.$data['numberofattendees'].'" />
						              <div class="wdf_eventlevel_title" rel="'.$data['title'].'">
								        <h3><input type="radio" class="wdf_eventlevel_pledge" name="wdf_pledge" rel="wdf_eventlevel_pledge_'.$index.'" value="'.$wdf->format_currency('',$data['amount']).'" /> '.$data['title'].'</h3>
								      </div>
						              <div class="wdf_eventlevel_amount" rel="'.$data['amount'].'">
						                <label>
						                  '.($settings['curr_symbol_position'] == 1 || $settings['curr_symbol_position'] == 2 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '').'
					                      Registration Fee:  '.$wdf->format_currency('',$data['amount']).'
					                      '.($settings['curr_symbol_position'] == 3 || $settings['curr_symbol_position'] == 4 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '').'
					                      '.($data['amounttype'] == 'per_person' ? 'per person' : '').'
					                    </label>
								      </div>
					                  <div class="wdf_eventlevel_numattendees">
								        <label>Number of Attendees:  '.$data['numberofattendees'].'</label>
								      </div>
					                  <div class="wdf_eventlevel_description">
								        <p>'.$data['description'].'</p>
								      </div>
                                    </div>';
					}
...
if(!function_exists('wdf_has_events')) {
	function wdf_has_events($post_id = '') {
		global $post;
		$post_id = (empty($post_id) ? $post->ID : $post_id );
		if(!get_post($post_id))
			return false;
		$meta = get_post_meta($post_id,'wdf_has_eventlevels',true);
...
if(!function_exists('wdf_get_page_link')) {
	function wdf_get_page_link($post_id, $type) {
		...
		switch($type) {
			...
			case'error' :
				if($permlink)
					$link = get_post_permalink($post_id) . $settings['error_slug'] .'/';
				else
					$link = add_query_arg( array('funder_confirm' => '1'), get_post_permalink($post_id) );
				break;
				default:
...
if(!function_exists('wdf_thanks_panel')) {
	function wdf_thanks_panel( $echo = true, $post_id = '', $trans = '' ) {
		...
		if($funder = get_post($post_id) && !empty($trans)) {
            ...
            if (isset($trans['recurring'])) {
            	$start_date = strtotime(str_replace("~", "/", $trans['RCRStartOnDay']));
            	switch ($trans['cycle']) {
            		case 'Weekly':
            			$day_of_week = date("l", $start_date);
            			$charge_date = "Every $day_of_week";
            			break;
            		case 'Monthly':
            			$month_date = date("jS", $start_date);
            			$charge_date = "On the $month_date every month";
            			break;
            		case '12Months':
            			$year_date = date("jS of F", $start_date);
            			$charge_date = "On the $year_date";
            			break;
            	}
            	$content .= '<h3 class="wdf_confirm_pledge_amount">Recurring Payment Information:</h3><br />
            			     <ul>
                             <li>Donation Frequency: '.$trans['cycle'].'</li>
                             <li>Donation Date: '.$charge_date.' starting today</li>
                             <li>Transaction ID: '.$trans['RecurID'].'</li>
                             </ul><br />';
            }
...
if(!function_exists('wdf_show_cancellation')) {
	function wdf_show_cancellation( $echo = true, $post_id = '', $checkout_step = '' ) {
		global $wpdb, $wdf;
        $success = false;
		if ( isset($_POST['cancel_pledge_id']) && isset($_POST['cancel_pledge_email'])) {

            // find donation
			$pledge = $wpdb->get_row("SELECT * FROM " . $wpdb->posts . " WHERE post_title = '" . $_POST['cancel_pledge_id'] . "' AND post_type = 'donation'");
			if ($pledge === false || !isset($pledge)) {
				$content = "<p style=\"color:red;\">Pledge ID not found! Please contact NNCCF for further instructions!.</p>";
				$success = false;
			}
			else {
			    $transaction = $wdf->get_transaction($pledge->ID);
			    if ($transaction === false || !isset($transaction['payer_email']) || $_POST['cancel_pledge_email'] != $transaction['payer_email'] ) {
				    $content = "<p style=\"color:red;\">Pledge email not found! Please contact NNCCF for further instructions!.</p>";
				    $success = false;
			    }
                else if (!isset($transaction['recurring'])) {
                    $content = "<p style=\"color:red;\">Recurring donation not found! Please contact NNCCF for further instructions!.</p>";
                    $success = false;
                }
                else {
                	// Update the post to reflect cancellation of recurring donation
                	$post_data = array (
                		                'ID'            => $pledge->ID,
                			            'post_status'   => 'wdf_canceled'
                	);
                	$_POST['post_status'] = 'wdf_canceled';
                	wp_update_post( $post_data );

                	$transaction_after = $wdf->get_transaction($pledge->ID);
                	if ($transaction_after['status'] == "Error") {
                		$content = "<p style=\"color:red;\">".$transaction_after['gateway_msg']."</p>";
                		$success = false;
                	}
                	else {
                		$content = "<p>Cancellation of your recurring donation complete. Thank you for your contribution!</p>";
                		$success = true;
                	}
                }
			}
		...
		if ($success == false) {
			$content .= '
			    <h3>Please enter your recurring donation information below.</h3>
				<form action="" method="post">
                  <table>
					<tr>
					  <td style="width:300px;">Your email that was used to setup the donation:</td>
					  <td><input type="input" size="40" name="cancel_pledge_email" value="" /></td>
					</tr>
					<tr>
					  <td>The pledge ID that was sent to you for this donation:</td>
					  <td><input type="input" size="12" maxlength="12" name="cancel_pledge_id" value="" /></td>
					</tr>
				    <tr>
					  <td colspan=2>
					    <label>Note that this information is displayed on the thank you page when you first setup the donation and also included in the email that was sent to the email you entered when you setup the donation.</label>
					  </td>
					</tr>
                  </table>
                  <br />
...
if(!function_exists('wdf_pledge_button')) {
	function wdf_pledge_button($echo = true, $context = '', $post_id = '', $args = array()) {
		...
		if($context == 'widget_simple_donate') {
			...
			$content .= '<input type="hidden" name="wdf_step" value="" id="wdf_step" />';
            ...
            $content .= '<div class="wdf_donation_info">';

			switch ($meta['wdf_type'][0]) {
				case 'simple':
					$content .= '<div class="wdf_simple_donation_label">'.apply_filters('wdf_choose_amount_label',__('Donation Information','wdf')).'</div>
                                 <div class="wdf_simple_donation_amount">
							         <label>Please enter your donation amount: </label>
							         '.($settings['curr_symbol_position'] == 1 || $settings['curr_symbol_position'] == 2 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '').'
					                 <input type="text" name="wdf_pledge" class="wdf_pledge_amount" value="" />
					                 '.($settings['curr_symbol_position'] == 3 || $settings['curr_symbol_position'] == 4 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '').'
					             </div>';
					break;

				case 'fixed':
					$content .= '<input type="hidden" name="wdf_pledge" value="'.$meta['wdf_fixed_amount'][0].'" />
				                 <div class="wdf_fixed_donation_label">'.apply_filters('wdf_choose_amount_label',__('Donation Information','wdf')).'</div>
							       <div class="wdf_fixed_donation_amount">
							         '.($settings['curr_symbol_position'] == 1 || $settings['curr_symbol_position'] == 2 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '').'
					                 <span class="wdf_pledge_amount">'.$meta['wdf_fixed_amount'][0].'</span>
					                 '.($settings['curr_symbol_position'] == 3 || $settings['curr_symbol_position'] == 4 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '').'
                                     will be charged to your account today.
					             </div>';
					break;

				case 'event':
					if(wdf_has_events($post_id)){
						$content .= '<div class="wdf_event_donation">'.wdf_events(false, $post_id).'</div>';
					}

                    $contact_info = maybe_unserialize($meta['wdf_contact_info'][0]);
					$content .= '<br /><div class="wdf_eventcontact_info" rel="wdf_eventcontact_info">
							       <h4>'.__('Contact Information','wdf').'</h4>
							       <p>Please provide us with primary contact person</p>
                                   <table id="wdf_eventcontact_table" class="widefat">';
                    foreach($contact_info as $name => $data) {
                    	if ($data == 'true') {
                    		$text_name = ucwords(substr($name, 8));
                    	    $content .= '<tr>
                                           <td><label class="wdf_event_contact_title">'.$text_name. '</label></td>
                                           <td><input type="text" class="wdf_event_contact_value" name="wdf_'.$name.'" value="'.$meta['wdf_'.$name].'" /></td>
                    	                 </tr>';
                    	}
                    }
                    $content .= '  </table>
                    ...
                    $content .= '<br /><div class="wdf_eventattendee_info" rel="wdf_attendee_info"></div>';

                    $attendee_info = maybe_unserialize($meta['wdf_attendee_info'][0]);
                    $content .= '<table class="wdf_eventattendee_table_template" class="widefat" style="display:none">
                    		       <tr>';
                    foreach($attendee_info as $name => $data) {
											if ($data == 'true') {
                    		$text_name = ucwords(substr($name, 9));
                    	    $content .= '<td><label class="wdf_event_attendee_title">'.$text_name. '</label></td>
                                         <td><input type="text" class="wdf_event_attendee_value" name="wdf_'.$name.'" value="'.$meta['wdf_'.$name].'" /></td>';
                    	}
									}
                    $content .= '  </tr>
                    		     </table>';
                ...
                case 'advanced':
					$content .= '<div class="wdf_advanced_donation_info">
					               <div class="wdf_advanced_donation_label">'.apply_filters('wdf_choose_amount_label',__('Choose An Amount','wdf')).'</div>
					                 '.($settings['curr_symbol_position'] == 1 || $settings['curr_symbol_position'] == 2 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '').'
				                     <input type="text" name="wdf_pledge" class="wdf_pledge_amount" value="" />
					                 '.($settings['curr_symbol_position'] == 3 || $settings['curr_symbol_position'] == 4 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '');
					break;
			...
			if (isset($meta['wdf_recurring'][0]) && isset($meta['wdf_type'][0]) &&
			    ($meta['wdf_type'][0] == 'simple' || $meta['wdf_type'][0] == 'fixed' || $meta['wdf_type'][0] == 'event') ) {

				if( ($meta['wdf_recurring'][0] == 'yes' || $meta['wdf_recurring'][0] == 'only') &&
				    (isset($meta['wdf_fixed_recurrance'][0]) && $meta['wdf_fixed_recurrance'][0] == 'A') ) {

					$content .= '<div class="wdf_recurring_info">
							       <label>Make this donation</label>
                                   <select class="wdf_recurring_type" name="wdf_recurring">
			    ...
			    else if ( ($meta['wdf_recurring'][0] == 'yes' || $meta['wdf_recurring'][0] == 'only') &&
				    (isset($meta['wdf_fixed_recurrance'][0]) && $meta['wdf_fixed_recurrance'][0] != 'A') ) {

			    	switch ($meta['wdf_fixed_recurrance'][0]) {
			    		case 'D':
			    			$recurrance = 'a daily';
			    			break;
			    		case 'W':
			    	    ...
			    	    default:
			    	    	$recurrance = 'a monthly';
			    	    	$meta['wdf_fixed_recurrance'][0] = 'M';
			    	    	break;
			    	}

			        $content .= '<input type="hidden" name="wdf_recurring" value="'.$meta['wdf_fixed_recurrance'][0].'" />
			        		     <div class="wdf_recurring_info">
			        		       <p>Additionally, this donation will recur on '.$recurrance.' basis until cancelled by you.</p>
			        		     </div>
			    	             ';
			    }
			}

			if ($meta['wdf_type'][0] == "simple" || $meta['wdf_type'][0] == "fixed") {
				$additional_info = maybe_unserialize($meta['wdf_additional_info'][0]);
				if (is_array($additional_info) && in_array('true',$additional_info) === true ) {
					if (array_key_exists('tribute_donation_info',$additional_info) && $additional_info['tribute_donation_info']=='true')	{
						$content .= '<br /><div class="wdf_additional_info" rel="wdf_additional_info">
							           <h4>'.__('Tribute Information','wdf').'</h4>
							           <p>Please provide us with the following information about your tribute donation.</p>
                                       <table id="wdf_additionalinfo_table" class="widefat">
						                 <tr>
                                           <td><label class="wdf_additional_title">Name of individual you are honoring: </label></td>
                                           <td><input type="text" class="wdf_additional_value" name="wdf_tibute_honoree" value="'.$meta['wdf_tibute_honoree'].'" size="40" /></td>
                                         </tr>
                                         <tr>
                                           <td><label class="wdf_additional_title">Occasion (birthday, anniversary, etc.) : </label></td>
                                           <td><input type="text" class="wdf_additional_value" name="wdf_tibute_occasion" value="'.$meta['wdf_tibute_occasion'].'" size="20" /></td>
                                         </tr>
                                         <tr>
																			   	<td><label class="wdf_additional_title">Honoree Shipping Address: </label></td>
                                           <td><textarea rows="3" cols="40" name="wdf_tibute_honoree_adddress" value="'.$meta['wdf_tibute_honoree_adddress'].'">Enter Address of honoree here...</textarea></td>
                                         </tr>
                                       </table>
                                     </div>';
					}
					if (array_key_exists('donor_info',$additional_info) && $additional_info['donor_info']=='true')	{
						$content .= '<br /><div class="wdf_additional_info" rel="wdf_additional_info">
												<h4>Donor Information</h4>
												<p>Please provide us with the following information about yourself.</p>
																			<table id="wdf_additionalinfo_table" class="widefat">
														<tr>
																					<td><label class="wdf_additional_title">Your Name: </label></td>

																					<td><input type="text" class="wdf_additional_value" name="wdf_donor_name" value="'.$meta['wdf_donor_name'].'" size="40" /></td>
																				</tr>
																				<tr>
																				<td><label class="wdf_additional_title">Your Shipping Address: </label></td>

																					<td><textarea rows="3" cols="40" name="wdf_adddress" value="'.$meta['wdf_adddress'].'">Enter Your Shipping Address Here</textarea></td>
																				</tr>
																			</table>
																		</div>';
					}
            ...
            $content .= '<div class="wdf_submit_donation"><input class="wdf_send_donation" type="submit" name="wdf_send_donation" value="'.$pledge_label.'" /></div>';
		}
...
if(!function_exists('wdf_eproc_pledge_button')) {
	function wdf_eproc_pledge_button($echo = true, $context = '', $post_id = '', $args = array()) {
		global $wdf; $content = '';
		...
		if($context == 'widget_simple_donate') {
			...
			switch ($meta['wdf_type'][0]) {
				...
				case 'fixed':
					$content .= '<div class="wdf_custom_donation_label">'.apply_filters('wdf_choose_amount_label',__('Donation Amount','wdf')).'</div>';
					$content .= ($settings['curr_symbol_position'] == 1 || $settings['curr_symbol_position'] == 2 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '');
					$content .= '<input type="text" name="wdf_pledge" class="wdf_pledge_amount" readonly="readonly" value="'.$meta['wdf_fixed_amount'][0].'" />';
					$content .= ($settings['curr_symbol_position'] == 3 || $settings['curr_symbol_position'] == 4 ? '<span class="currency">'.$wdf->format_currency().'</span>' : '');
					break;

				case 'event':
					if(wdf_has_events($post_id)){
						$content .= '<div>'.wdf_events(false, $post_id).'</div>';
					}
					break;
			...
			($meta['wdf_type'][0] == 'simple' || $meta['wdf_type'][0] == 'fixed' || $meta['wdf_type'][0] == 'event') ) {
				...
				(isset($meta['wdf_fixed_recurrance'][0]) && $meta['wdf_fixed_recurrance'][0] == 'A') ) {
					...
					$content .= '<label>Make this donation </label>
				...
				(isset($meta['wdf_fixed_recurrance'][0]) && $meta['wdf_fixed_recurrance'][0] != 'A') ) {
					...
					$content .= '<label>This donation will recur </label>
			    	             <input type="text" name="wdf_recurring" readonly="readonly" value="'.$recurrance.'" />
			    	             <label> until cancelled</label>';
				}
			}
