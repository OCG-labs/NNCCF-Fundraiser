...
class WDF {
	...
	function _construct() {
		...
		if(is_admin()) {
			...
			add_filter('add_post_metadata', array(&$this, 'add_custom_fields'), 10, 6);
	...
	function template_redirect() {
		...
		if ($wp_query->query_vars['post_type'] == 'funder') {
			...
			if ($funder_name == 'cancel-recurring') {
				$this->is_funder_cancel_recur = true;
				if ( $funder_id )
			...
			elseif (isset($wp_query->query_vars['funder_checkout']) && $wp_query->query_vars['funder_checkout'] == 1) {
	...
	function funder_content($content) {
		...
		if(isset($this->is_funder_cancel_recur) && $this->is_funder_cancel_recur) {
			$content = wdf_show_cancellation( false, $post->ID, (isset($_POST['wdf_step']) ? $_POST['wdf_step'] : '')  );
		}
	...
	function add_meta_boxes() {
		...
		if($typenow == 'funder') {
			...
			if($wdf_type != false && !empty($wdf_type)) { // We have a type so show the available options
				...
				add_meta_box('wdf_specific', __('Custom Fields'), array(&$this,'meta_box_display'), 'funder', 'normal', 'high');
			}
				...
				if($value != '') {
					if($key == 'goal') {
					    ...
					    foreach($value as $name => $data) {
						    $value[$name] = esc_textarea($data);
						}						
                    } else if($key == 'attendee_info') {
					    foreach($value as $name => $data) {
						    $value[$name] = esc_textarea($data);
						}		
					...
					} else if($key == 'eventlevels') {
						foreach($value as $level => $data) {
                            $value[$level]['title'] = esc_textarea($data['title']);
							$value[$level]['amount'] = $this->filter_price($data['amount']);
							$value[$level]['amounttype'] = esc_textarea($data['amounttype']);
							$value[$level]['numberofattendees'] = esc_textarea($data['numberofattendees']);
							$value[$level]['description'] = esc_textarea($data['description']);
						}
	...
	function edit_columns($columns) {
		...
		if($typenow == 'funder') {
			...
			$columns['gateway_status'] = __('Gateway Status', 'wdf');
	...
	function column_display($name) {
		...
		switch($name) {
			...
			case 'pledge_status' :
				...
				$status = ucfirst(str_replace('wdf_','',$status));
				echo $status;
				break;
		    case 'gateway_status' :
                $trans = $this->get_transaction($post->ID);
                if ($trans['status'] == 'Error') {
                    echo $trans['gateway_msg'];
                }
                else {
                    echo $trans['status'];
                }
	...
	function add_custom_fields($value, $object_id, $meta_key, $meta_value, $unique){
/*
        global $wpdb;

        if (strpos($meta_key,'wdf_cst_') === false) {
            $meta_key = 'wdf_cst_'.$meta_key;
        }
        
        $result = $wpdb->insert('wp_postmeta', array('post_id' => $object_id,'meta_key' => $meta_key,'meta_value' => $meta_value));
        if ($result === false) {
            return false;
        }
        else {
